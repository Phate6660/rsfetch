...
pub async fn async_main(matches: ArgMatches<'_>) {
    ...
    let f7 = async_memory(&matches);
    let f8 = async_packages(&matches);
    let f9 = async_shell(&matches);
    let f10 = async_uptime(&matches);
    let f11 = async_user(&matches);
    let f12 = async_music(&matches);
    futures::join!(f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12);
}
